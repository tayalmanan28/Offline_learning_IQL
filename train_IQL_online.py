...
if __name__ == "__main__":
    ...
    parser.add_argument("--env", type=str, default="halfcheetah-expert-v2")  # OpenAI gym environment name  walker2d-bullet-medium-v0
    ...
    file_name = f"{args.policy}_{args.env}_{args.expectile}"
    print("---------------------------------------")
    ...
    dataset = env.get_dataset()#h5py.File(os.path.join(args.data_path, args.env + '.hdf5'), 'r')
    ...
    for epoch in range(-n_epochs, n_epochs):
        ...
        if epoch<0:    
            ...
            plt.xlabel('Gradient Steps (x10^4)')
            plt.title(file_name)
